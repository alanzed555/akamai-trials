const KeysMap = {
    ...
    '-1,2, -94, -102,': 'inform_info',  // Log in with form to have data
    ...
    '-1,2, -94, -121,': 'footer',  // Finally, some time parameters
    // '-1,2,-94,-120,': '',
};
...
export class WebAkamai {
    ...
    static splitStr(str) {
        ...
        const bmak = {
            see: '1.43',
            sensor_status: map.sensor_status,  // Static data
            ...
            ...Webmaster.parseAjCounter(map.aj_counter),
            ...Webmaster.parseFpBmak(map.fp_val),
            ...Webmaster.parseO9AndD3Bmak(map),
            ...Webmaster.parseKeyboardEventsForBmak(map),
            ...Webmaster.parseMouseEventsForBmak(map),
            ...Webmaster.parseTouchEventsForBmak(map),
            ...Webmaster.parseDmtionForBmak(map),
            ...Webmaster.parseDOrienForBmak(map),
            ...WebAkamai.parsePointerForBmak(map),
            ...Webmaster.parseDChangeForBmak(map),
            ...Webmaster.parseFooterForBmak(map),
            ...Webmaster.parseCookiesBmak(map.Cookies),
        };
        map.tst = bmak.tst;
        map.ver = bmak.give;
        map.bmak = bmak;
    ...
    static packObj(obj) {
        ...
        sensor_data + =  '-1,2, -94, -102,' + obj.inform_info;  // Log in if there is data
        ...
        const w = WebApplication.ab(sensor_data);
        ...
        const end = WebApplication.getHeaderAndFooter(obj);
        sensor_data + =  '-1,2, -94, -121,' + end.footer;  // Finally, some time parameters
        sensor_data = end.header.replace(obj.ver, '') + sensor_data;
        // Still need to generate dynamically
    ...
    bmak = {
        see: '1.43',
        aj_type: 0, // more important
        aj_index: 0,  // ajax counter
        sensor_status: 'do_en, dm_en, t_en',  // Static data
        current_url: 'https://s3.nikecdn.com/unite/mobile.html',
        env_config: '0,0,0,0,1,0,0',  // Another wave of detection environment
        inform_info_pre: '',
        inform_info: '',
        mn_r_1: '',
        mn_r_2: '',
        ...
        init_time: 0,
        tst: 5,  // The time spent in initialization is generally between 6-10

        // will eventually be passed by cookie
        canvasFp1: '-1',  // Configure fingerprint
        canvasFp2: '-1',
        fp2Val: '-1',

        runFonts: 'dis',  // The default dis is fine, if bmak ['altFonts'] == true, then there is data
        ...
        fpcf: {
            td: 6, // The time it takes to get some information, just write it directly
        },
        //2.x
        colorDepth: 24,
        pixelDepth: 24,
    ...
    prod: 'Gecko',
        ...
        pluginInfo: '',  // default is empty, generally empty,
        plen: 0, //plugins.length
        pen: 0, //_phantom
        wen: 0, //webdriver
        den: 0,  // domAutomation
        z1: 0,  // Need to calculate
        ...
        o9: '-1',
        d3: '-1',
        ...Webmaster.getO9AndD3(),

        //cookies
        n_ck: 0,  // Whether it is a new ck
        d2: 0,
    ...
    constants = {
        ...
        s3_login_url: ' https : //s3.nikecdn.com/unite/mobile.html ',  // web edge url
    };
    ...
    static parseMouseEventsForBmak(map) {
        ...
        return {
            mouse_action: {
                ...
                me_cnt: me_cnt, // counter will accumulate
                me_vel: me_vel,
                ta: ta,
            },
        };
    ...
    static parseCookiesBmak(str) {
        ...
        const map = {
            ...
            canvasFp2: list[23],
            fas: list[24],
            // Initialize some event value data
            fpcf: {
                rVal: list[22],
                rCFP: list[23],
                td: list[10],
    ...
    static getUserAgentStr(bmak) {
        let happen outside = max.uar;
        bmak.z1 = parseInt(bmak.start_ts / (2016 * 2016) + '');
        const r1 = WebApplication.randLNm();
        const list = [
            uar,
            'uaend',
            bmak.to,
    ...
    static getCookiesStr(bmak) {
        ...
        the let T = a Date .now ( ) - bmak.Start_TS +  . 1 -  . 4; // fingerprint calculation times, more time consuming, if debug mode -6
        bmak.d2 = parseInt(bmak.z1 / 23 + '');
        let l = t + 1; // The time calculated above, in fact, no other is needed here
        ...
        const et = parseInt(bmak.keyboard_action.ke_vel)
            ...
            + parseInt(bmak.pointer_action.pe_vel); // Total number of events
        ...
        const list = [
            ...
            et,  // Top 6
            t,  // Start time difference, start time-current time
            bmak.init_time,  // Basically: 0
            bmak.start_ts, //init_time
            parseInt(Math.random() * 5 + 3 + ''),  // Fingerprint production time td
            bmak.d2,  // z1 / 23
            bmak.keyboard_action.ke_cnt,  // ke_cnt
            bmak.mouse_action.me_cnt, //me_cnt
            parseInt(bmak.d2 / 6 + ''),  // s fixed value
            bmak.pointer_action.pe_cnt,  // pe_cnt
            bmak.touch_action.te_cnt, //te_cnt
            l,  // event
            bmak.ta,  // ta sensor data is 18 OK
            bmak.n_ck,  // whether it is a CK 19 produced
            cks, //e 20
            WebAkamai.Ab(CKS), // encrypted cookie value
            bmak.fpcf.rVal,  // random number produced
            bmak.fpcf.rCFP, // fingerprint
            30261693,  // fas, it should be important to indicate some value 24
        ];
        return list.join(',');
    }
    ...
    /**
     * Parse obj into sensor object data and pass it to static method
     * @param bmak
     * @returns {string|*}
     */
    static packBMakToObject(bmak) {
        return {
            ...
            cookies: Webmaster.getCookiesStr(bmak),
            aj_counter: WebApplication.getAjCounter(bmak),
            ...
            env_config: bmak.env_config,  // Another wave of detection environment
            mn_r_1: bmak.mn_r_1,
            mn_r_2: bmak.mn_r_2,
            fp_val: WebAkamai.getFpVal(bmak),
            start_time: bmak.start_time,  // This value is o9 value
            ...Webmaster.getHeaderAndFooter(bmak),
        };
    }
    ...
    /**
     * It needs to be written during initialization, this should be a data verification
     */
    static getO9AndD3() {
        let a = this.get_cf_date() % 1e7;  // Remainder of the current time% 7
        const d3 = a;
        let t = a;
        for (let e = 0; e < 5; e++) {
    ...
    /**
     * For verification, two data are returned, one is header and one is footer
     * @param bmak
     */
    static getHeaderAndFooter(bmak) {
        const uptime = WebApplication.get_cf_date() - 2;
        const j = Webmaster.get_cf_date();
        const C = Math.floor(WebAkamai.get_cf_date() / 3600000);  // Parameter value that will not change within an hour
        const head = WebApplication.od(WebApplication.cs, WebApplication.api_public_key).slice(0, 16);
        const E = head + WebAkamai.from(C, head);
        let footer = ';' + (WebApplication.Get_cf_date() - uptime) + ';' + bmak.tst + ';' + (WebApplication.Get_cf_date() - j);
        return {
            header: E,
            footer: footer,
        };
    ...
    create() {
        ...
        const sensors = [];
        // 3. Submit some events and click
        bmak.aj_type = 1;
        bmak.aj_index = 4;
        ...
        return sensors.map(ss => {
            const obj = WebA Brother .packBMakToObject (ss);
